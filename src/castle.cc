...
void Castle::activate()
{
  ...
  foreach (Player* p, players_)
  {
    ...
    do
    {
      ...
      {
        //worker_->printMessage("No food cube.");
        can_pay = false;
      }
      else if (!_checkResources(*p))
      {
...
void Castle::_build(Player* p, std::pair<Resource, Resource> resources)
{
  ...
  p->resources() -= resources.first;
  p->resources() -= resources.second;
